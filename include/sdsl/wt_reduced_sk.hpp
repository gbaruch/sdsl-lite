...
if(bv.size() > 0)
{
	...
	/*
        ...
        value_type operator[](size_type i)const {
            ...
            //add the pruned bits
            uint64_t pruned =0 ;
            if(h != (node_type)-1/* && h != 0*/)
            {
            	pruned = m_leaves_bv.get_int(this->m_tree.bv_pos(v) + h * i, h);
                uint64_t char_index = this->m_tree.bv_pos_rank(v) + pruned;
                return m_pruned_chars[char_index];
            }
            else
            {
		...
		virtual size_type serialize(std::ostream& out, structure_tree_node* v=nullptr,
			...
			written_bytes += this->m_tree.serialize(out,child,"tree", false);
			written_bytes += this->m_leaves_bv.serialize(out,child,"leaves_bv");

			written_bytes += write_member(this->m_pruned_chars.size(), out, child, "m_pruned_chars.size()");
			written_bytes += serialize_vector(this->m_pruned_chars,out,child, "m_pruned_chars");
		...
		virtual void load(std::istream& in) {
			...
			this->m_tree.load(in, false);
			this->m_leaves_bv.load(in);

	        uint64_t m_pruned_chars_size = 0;
	        read_member(m_pruned_chars_size, in);
	        m_pruned_chars = std::vector<value_type>(m_pruned_chars_size);
	        load_vector(m_pruned_chars, in);
