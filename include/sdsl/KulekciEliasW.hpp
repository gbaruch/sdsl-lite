#ifndef INCLUDED_KulekciEliasW
#define INCLUDED_KulekciEliasW
...
class KulekciEliasW
{
    public:
        ...
        typedef random_access_const_iterator<KulekciEliasW> const_iterator;
    ...
    protected:
        ...
        wt_type m_wt;
        bit_vector m_rawBits[64];//there can be at most 64 different labels 0..63
        uint64_t m_size;

        void copy(const KulekciEliasW& wt) {
        	m_wt      = wt.m_wt;
    ...
    KulekciEliasW(int_vector_buffer<int_width>& buf, size_type size)  {
		...
		{
			//compute the label
			labels[i]=bits::hi(buf[i]+2);//we encode x+2 to get rid of an if statement in decoding, which helps us to perform faster

			bitvectorSize   = m_rawBits[labels[i]].size();//current size of the corresponding rawbits

			//compute the payload, which is the least significant labels[i] bits. Append these bits to the rawBits[label]
			m_rawBits[labels[i]].resize(bitvectorSize+labels[i]);
			for(j=0; j<labels[i]; j++) m_rawBits[labels[i]][bitvectorSize+j] = (buf[i]+2) & (one<<j);//insert from least significant to most significant to be able to retrieve in one 64-bit read operation
		}
        construct_im(m_wt, labels);//create the huffman shaped wavelet tree of the labels
        for(int i =0 ; i<64;i++)
        {
        	m_rawBits[i].resize(m_rawBits[i].size()+64);//pad 64bits
        }
    }
    ...
    value_type operator[](size_type i)const {
        ...
        uint64_t one = 1, labelBits=0;
		auto rc   = m_wt.inverse_select(i);
		j         = rc.first;
		label     = rc.second;
		labelBits = m_rawBits[label].get_int(j*label);
		value     = (labelBits & ((one<<label)-1));
		value    |= (one<<label);
		return value - 2;
	};

        //! Default constructor
        KulekciEliasW() {m_size = 0;
        };



        //! Copy constructor
        KulekciEliasW(const KulekciEliasW& wt) {
            copy(wt);
        }

        //! Copy constructor
        KulekciEliasW(KulekciEliasW&& wt) {
            *this = std::move(wt);
        }

        //! Assignment operator
        KulekciEliasW& operator=(const KulekciEliasW& wt) {
            if (this != &wt) {
                copy(wt);
            }
            return *this;
        ...
        KulekciEliasW& operator=(KulekciEliasW&& wt) {
            if (this != &wt) {
        ...
        //! Swap operator
        void swap(KulekciEliasW& wt) {
            if (this != &wt) {
                m_wt.swap(wt.m_wt);

                for(int i =0 ; i < 64; i++){
                	m_rawBits[i].swap(wt.m_rawBits[i]);
                }
            	m_size = wt.m_size;
            }
        }
        ...
        size_type serialize(std::ostream& out, structure_tree_node* v=nullptr, std::string name="")const {
            ...
            for(int i =0 ; i < 64; i++){
            	written_bytes += m_rawBits[i].serialize(out, child, "m_rawBits");
            }
